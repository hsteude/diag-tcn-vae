...
class DiagTsDataset(Dataset):
    def __init__(self, seq_len: int, data_path: str, cols: list[str]):
    ...
    def __getitem__(self, index):
        return (
            ...
            self.x[index + self.seq_len : index + 2 * self.seq_len, :].T,
        )
...
class DiagTsDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.dataset = DiagTsDataset(
            data_path=self.data_dir, seq_len=self.seq_len, cols=self.cols
        )
