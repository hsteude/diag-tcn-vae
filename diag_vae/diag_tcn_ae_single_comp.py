...
class DiagTcnAeSingleComp(pl.LightningModule):
    def __init__(
        self,
    ...
    def predict_step(self, batch, batch_idx: int, dataloader_idx: int = 0):
        ...
        comp_mse_ls = [
            ...
            for x, x_hat in zip([x_ls[self.comp_num]], x_hat_ls)
        ]
        return comp_mse_ls

    def loss_function(self, x_ls, x_hat_ls):
        # breakpoint()
        x = torch.cat(x_ls, dim=1)
        x_hat = torch.cat([x_hat_ls[self.comp_num]], dim=1)
